...
import ParagraphGenerator from './components/paragen';
import './App.css';
...
const App = () => {
  ...
  const handleGenerateClick = () => {
    const paragraphs = [];
    if(numberOfParagraphs>8 || numberOfParagraphs <1)
    window.alert("Enter number between 1 to 8!");
  else{
    for (let i = 0; i < numberOfParagraphs; i++) {
      const paragraphContent = paragraphData[i];
  ...
  return (
    <div>
      ...
      <label htmlFor="paragraphInput">Number of Para:</label>
      <input
        type="number"
        id="paragraphInput"
      ...
      <ParagraphGenerator data={generatedParagraphs} />
    </div>
  );
};
